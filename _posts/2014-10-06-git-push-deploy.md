...
This has been written about by a number of people already, but I'd like to share my experiences with it so far, and some of the cool solutions I used.
...
The code for this article is [available at Github](https://github.com/geon/deploy-demo). 

Installing the server
---------------------

I won't go through the entire process of installing the server. I used a pretty standard install of Ubuntu 14.10 with PostgreSQL and Node.js. The rest of the article assumes you have it set up with a user named `demouser`.
...
At first, it used the shell initialization script of a specific user that was made to auto-login on boot, and the Node.js based Forever. It was a huge cludge, made overcomplicated by some other features the app had to support.
...
* Logging to the right file - or you won't know what went wrong.
...
Setting up the repo on the production server is pretty simple. I use separate folders for the script-files and the repo itself, placed right in the user's home folder. Just

	mkdir deploy-demo
	mkdir deploy-demo.git

and initialize the folder with the `.git` suffix as "bare". The suffix is nothing magical, just a convention.
...
Copy them:	
	...
	cp ~/deploy-demo/setup/git-hooks/* ~/deploy-demo.git/hooks
...
if [ "0" == $(expr "$oldrev" : "0*$") ]; then
	echo -e "\n\033[1;31m - Changes to schema.sql - Update the database to match! - \033[0m\n"
	git diff --color $oldrev $newrev -- setup/schema.sql setup/setup-raw.sql

	echo -e "\n\033[1;31m - Changes to config.js - Update manually! - \033[0m\n"
	git diff --color $oldrev $newrev -- config.js
fi
```

The `if` ensures the diffs are not run on the first push, since there is nothing to diff against yet.
...
Checking out the files overwrites any local changes, but leaves any new files, like caches, uploaded user files, etc. alone. Make sure you never check these files in. I like to make that easier by storing them all in a `unversioned-files` folder in the project root. It is checked in, but added to the `.gitignore` file, and any other folder that need to have unversioned files (like the `public` folder in Express) are symbolic links into it.
...
Changes you do locally to this file should not normally be committed. The the content of the file for instructions on how to handle that.
...
To make your app work with Upstart, the Upstart file needs to be copied to `/etc/init/`.
	...
	sudo cp ~/deploy-demo/setup/deploy-demo.conf /etc/init/
...
How about changing the table definiton in `setup/schema.sql`? Add a collumn for author id, and remove the keywords.
...
remote: 
remote: <b>diff --git a/setup/schema.sql b/setup/schema.sql</b>
remote: <b>index 873c3ef..e2ea75e 100644</b>
remote: <b>--- a/setup/schema.sql</b>
remote: <b>+++ b/setup/schema.sql</b>
